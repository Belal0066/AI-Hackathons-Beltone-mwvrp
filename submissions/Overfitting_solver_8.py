...
def beam_search_path(
    ...
    """Compute a path using Beam Search between start and goal.
    Keeps only the best `beam_width` partial paths at each expansion step.
    """
    if start == goal:
        return [start]
    ...
    while queue:
        ...
        # Expand all paths at this level
        while queue:
            path = queue.popleft()
            node = path[-1]
            if node == goal:
        ...
        # Sort by heuristic to goal (lower = better)
